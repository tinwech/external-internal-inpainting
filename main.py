...
parser.add_argument('--pyramid_height', type=int,
                    default=3, help='pyramid_height ')
parser.add_argument('--save_every', type=int, default=200,
                    help='iterations to save')
parser.add_argument('--img_path', type=str,
                    default="images/input.jpg", help='path to input')
parser.add_argument('--gray_path', type=str,
                    default="images/gray.jpg", help='path to gray image')
parser.add_argument('--mask_path', type=str,
                    default="images/mask.png", help='path to mask')
parser.add_argument('--result_path', type=str,
                    default="results/", help='path to saved_result')
...
def get_pyramids(mask, img, img_gray, pyramid_height, dtype):
    ...
    for k in range(pyramid_height):
        ...
        img_gray_var_pyr.append(get_pyramid_image(
            img_gray, w_n, h_n)[:, :1, :, :].type(dtype))
        img_var_pyr.append(get_pyramid_image(img, w_n, h_n).type(dtype))
        mask_var_pyr.append(mask_var.type(dtype))
        mask_var = (1 - torch.nn.functional.max_pool2d((1-mask_var),
...
def train(pyramid_height, mask, img, img_gray, dtype, opt):
    ...
    mask_var_pyr, img_var_pyr, img_gray_var_pyr = get_pyramids(
    ...
    for j in range(pyramid_height):
        ...
        if j == 0:
            ...
            out = np.clip(out.detach().cpu().numpy()[
                          0]*255, 0, 255).astype(np.uint8)
            out = Image.fromarray(out.transpose(1, 2, 0))
            out_resized = out.resize((img_gray_var_pyr[j].size(
                3), img_gray_var_pyr[j].size(2)), Image.LANCZOS)
            out_resized_var = torch.from_numpy(
                np.array(out_resized).transpose(2, 0, 1) / 255.)[None, :].type(dtype)
        ...
        for i in range(num_iter[j]):
            ...
            if i % opt.save_every == 0:
                out_ = np.transpose(
                    out.detach().cpu().numpy()[0]*255, [1, 2, 0])
                out_ = Image.fromarray(out_.astype('uint8'), 'RGB')
                out_.save("%s/scale_%d_iter_%05d_out.jpg" %
                          (opt.result_path, j, i))
